...
- **`sqlite.go`** — the module entry. `Module` holds a `base.ConfigurableModule`; `NewModule()` / `NewModuleWithFileAccess(allowed)` / `NewModuleWithHostPolicy(policy)` construct it. `LoadModule()` exposes three builtins: **`connect`** (local), **`connect_remote`** (libSQL, with `file:` routed to the local path), **`register_function`** (custom scalar funcs, rejected when disabled by the host). Config keys, policy flags and `isInMemoryDSN` live here.
...
3. **Opt-in file-access gate.** `NewModuleWithFileAccess(false)` sets `restrictFileAccess` and freezes the configured `database` at construction through `base.SetHostOnly`; no `set_database` is exported. `connect` (including `connect_remote` with a `file:` URL) rejects other disk paths; `attach` allows only memory. `isInMemoryDSN` trusts only the parsed empty/`:memory:` path, not query parameters such as `mode=memory`, because SQLite and Go parse malformed URI queries differently. Keep that strict classification.
...
6. **UDF register/connect serialization (PKG-29).** modernc's `Driver.Open` reads its process-global UDF map (`d.udfs`) with no lock, while `RegisterScalarFunction` (called by `register_function`) writes it. `connect` opens through `localDriverName` — a `funcMutex`-serialized wrapper (`ensureLocalDriver` / `udfSerializedDriver`) whose `Open` holds `funcMutex.RLock` while modernc reads the map, mutually exclusive with `register_function`'s `Lock`/write. This is required because a connection's `Open` is called lazily by `database/sql` (and from parallel host machines), so the "register before connect" convention doesn't hold. **Keep `connect` on `localDriverName`, never raw `"sqlite"`.** Scope/limits (documented in `ensureLocalDriver`): it protects this module's own register vs connect, not a different process-wide user of raw `"sqlite"`, nor collation/connection-hook registration (this module registers neither).
7. **Opt-in UDF exclusion (STAR-102).** `DisableCustomFunctions` uses `noFunctionsDriverName`, backed by an independent zero-value modernc driver with no registered functions, collations or hooks. It also rejects `register_function` before touching the global registry. Every local path, including the libSQL `file:` fallback and pool replacement, must preserve this choice. The legacy path still uses `localDriverName`; network server functions remain server-controlled. This is exclusion, not isolated/budgeted execution of enabled UDFs.
...
- **CI matrix** = `[1.20.x, 1.27.x]` via the centralized reusable workflow in `1set/meta`.
